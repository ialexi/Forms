...
*/
Forms.FormView = SC.View.extend(
  SC.Editable,
/** @scope Forms.FormView.prototype */ {
  
  classNames: 'sc-form-view'.w(),
  
  concatenatedProperties: 'fields rowMixin fieldMixin formMixin'.w(),
...
//  content: null,
  ...
  */
  _updateFields: function()
  {
    ...
    for (var idx = 0; idx < fields.length; idx++)
    {
      ...
      {
        console.log('The key:', key, this[key]);
        view = this[key];
      }
  ...
  /**
    Adds a child view to the _needsLayout set blah blah...
    
    More importantly, gets the index of the view so next relayout will
    start there.
  */
  layoutDidChangeFor: function(childView)
  {
    ...
    // if it is a field...
    var index = this.get("_displayFields").indexOf(childView);
    if (index >= 0)
    {
      // then we need to invalidate from there on
      if (SC.none(this._invalidateFrom)) this._invalidateFrom = index;
      this._invalidateFrom = Math.min(index, this._invalidateFrom);
    }
  },
  ...
  */
  relayoutFields: function(startPoint)
  {
    var start = 0,
      currentY = 0,
    ...
    // first, if startpoint exists and is > 0... convenient.
    if (startPoint)
    {
      // return immediately if that is the last field: nothing to layout!
      if (startPoint == len - 1) return;
      
      var new_height;
      
      // get height of it
      if (displayFields[startPoint])
      {
        // ought to have a height value
        new_height = displayFields[startPoint].get("layout").height;
      }
      
      // normalize
      if (!new_height) new_height = 0;
      
      // calculate old height
      var old_height = fieldPositions[startPoint + 1] - fieldPositions[startPoint];
      
      // if not different, return
      if (old_height == new_height) return;
      
      start = startPoint;
      currentY = fieldPositions[start];
    }
